...
import com.yarenty.math.MatrixUtils
...
object SingularSpectrumTransformer {
  ...
  class Parameters(
                    ...
                    var n_dim: Int = 5,
                    var scoreFunc: ScoreFunction = SVD,
                    var changepointThreshold: Double = -1.0
                  ) {

    assert(scoreFunc != null, "Must specify score function")
    def set(func: ScoreFunction): Unit = {
      assert(scoreFunc != null, "Must specify score function")
      this.scoreFunc = func
    }
...
class SingularSpectrumTransformer(val params: SingularSpectrumTransformer.Parameters) {
  ...
  /**
    ...
    */
  private def computeScoreSVD( H: RealMatrix,  G: RealMatrix): Double = {
    assert(H != null , "SVD must have not null matrixes (H)")
    assert(G != null , "SVD must have not null matrixes (G)")
  ...
  /**
    ...
    *
    */
  private def computeScoreIKA( H: RealMatrix,  G: RealMatrix): Double = {
    assert(H != null , "SVD must have not null matrixes (H)")
    assert(G != null , "SVD must have not null matrixes (G)")
